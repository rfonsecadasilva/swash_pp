...
def Mfv_fig(ds,xmin=None,xmax=None,dx=None,ymin=None,ymax=None,dy=None,scale=None,vel_clip_max=None,vel_clip_min=None,dpi=100,plot_dep_dev=False,dep_levels=None,mfvy_levels=None,mfvy_ticks=None,cmap="RdBu_r"):
    """
    ...
    """
    warnings.filterwarnings('ignore')
    # remove Mfvy=0 (as this is a paralelisation issue)
    #ds=ds.where(lambda x:x["Mfvy"]!=0,drop=True)    
    # Assign xmin, xmax, dx, ymin, ymax, dy, tmin, tmax, and dt if not defined
    xmin = xmin or ds.x.min().item()
    xmax = xmax or ds.x.max().item()
    dx = dx or (ds.x.isel(x=1)-ds.x.isel(x=0)).item()
    ...
    ax[0].axis('equal')
    # plot hs
    mfvy=ds["Mfvx"].plot(ax=ax[0],cmap=cmap,levels=mfvy_levels,add_colorbar=False,clip_on=True)
