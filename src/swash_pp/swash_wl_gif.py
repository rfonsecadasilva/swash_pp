...
def create_gif(gif_name,ds,xmin,xmax,tmin,tmax,dt,zmin,zmax):
    """
    ...
    if not os.path.exists(gif_name): os.mkdir(gif_name)
    ...
    ibp=temp.Ibp # instantanenous beach position
    ...
    def fig_rp(t=0):
        ...
        plt.savefig(f'{gif_name}/{gif_name}_Fig_{t:04d}.png',dpi=100)
        plt.close() 
    ...
    def frames_gif(gf,dt):
        ...
        frames=[]
        for filename in [f'{gif_name}/{gif_name}_Fig_{t:04d}.png'  for t in range(gf)]:
            frames.append(imageio.imread(filename))
        imageio.mimsave(f'{gif_name}.gif', frames, 'GIF', duration=dt)          
